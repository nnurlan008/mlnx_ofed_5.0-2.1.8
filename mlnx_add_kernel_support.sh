...
if [ $skip_repo -eq 0 ]; then
	...
	ex /usr/bin/perl $tmpdir/$iso_dir/create_mlnx_ofed_installers.pl --with-hpc --tmpdir $tmpdir_logs --mofed $tmpdir/$iso_dir --rpms-tdir $tmpdir/$iso_dir/$BINDIR/UPSTREAM_LIBS --output $tmpdir/$iso_dir $DEBFLAG $KER_META_FLAG $META_BUILD_NUM $METAGROUPS --upstream-libs
