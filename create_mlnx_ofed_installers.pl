...
sub init_hash
{
	...
	for my $file (@files) {
		...
		for my $name (@names) {
			...
			if ($name =~ /fabric-collector|mlnx-nvme|libdisni|neohost-backend/) {
				next;
			}
	...
	if (not $isDeb) {
		...
		for my $rpm (<$RPMS/*rpm>) {
			...
			if ($kernel ne "" and
			    $name =~ /kernel|-modules$|^knem$|^iser$|^isert$|^srp$|^kernel-mft$|^gpio-mlxbf$|^i2c-mlx$|^mlxbf-livefish$|^mlx-bootctl$|^pka-mlxbf$|^tmfifo$|^rshim$|^mlx-trio$|^mlx-l3cache$|^mlx-pmc$/ and
			    not ($name =~ /mlnx-ofa_kernel-devel|mlnx-ofa_kernel$/) and
			    $ver !~ /$kernel/) {
				warn "$name $ver <> $kernel\n";
				next;
			} elsif ($kernel eq "" and
				 $name =~ /kernel|-modules$|^knem$|^iser$|^isert$|^srp$|^kernel-mft$|^gpio-mlxbf$|^i2c-mlx$|^mlxbf-livefish$|^mlx-bootctl$|^pka-mlxbf$|^tmfifo$|^rshim$|^mlx-trio$|^mlx-l3cache$|^mlx-pmc$/ and not ($name =~ /mlnx-ofa_kernel-devel|mlnx-ofa_kernel$|kmp|kmod/)) {
				next;
			}
...
## End of spec file template
		...
		$spec =~ s/\@REQUIRES\@//;
		if ($groups_info{$group_id}{'name'} =~ /^all$|^all-user-only$/x) {
